  ...
  return (
    <div class="flex gap-[0.25em]" ref={scope}>
      {Array.from({ length: 5 })
        ...
        .map((letter, i) => (
          <div key={i} class="relative h-[2em] w-[2em] font-bold uppercase">
            <div
              class={`letter-back absolute inset-0 flex items-center justify-center ${
                variants[letter.status]
              }`}
            >
              {letter.letter}
            </div>
